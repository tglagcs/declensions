...
(function () {
  ...
  options.forEach((opt) => {
    opt.addEventListener('click', (e) => {
      ...
      closeSelect();
      if (liveMode || fioInput.value.trim()) declineFullName();
    });
  });
  ...
  function declineFullName(animate = true) {
    ...
    const declined = [
      declineSurname(surname, c, gender),
      declineName(name, c, gender),
      declinePatronymic(patronymic, c, gender),
    ].filter(Boolean).join(' ');
  ...
  fioInput.addEventListener('input', () => {
    ...
    if (liveMode) declineFullName(false);
  });
